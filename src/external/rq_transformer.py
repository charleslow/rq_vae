"""
RQ-transformer implementation by lucidrains.
https://github.com/lucidrains/RQ-Transformer/blob/main/rq_transformer/rq_transformer.py
"""
...
def FeedForward(*, dim, mult=4, dropout=0.0):
    return nn.Sequential(
        nn.LayerNorm(dim), nn.Linear(dim, dim * mult), nn.GELU(), nn.Dropout(dropout), nn.Linear(dim * mult, dim)
    )
...
class Transformer(nn.Module):
    def __init__(self, *, dim, layers, dim_head=64, heads=8, attn_dropout=0.0, ff_dropout=0.0, ff_mult=4):
        super().__init__()
...
class RQTransformer(nn.Module):
    ...
    def forward(self, ids, return_loss=False):
        ...
        spatial_tokens = torch.cat((repeat(self.spatial_start_token, "f -> b 1 f", b=b), spatial_tokens), dim=-2)
        ...
        tokens_with_depth_pos = F.pad(tokens_with_depth_pos, (0, 0, 0, 0, 0, 1), value=0.0)
