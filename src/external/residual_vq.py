"""Residual Vector Quantizer implementation by lucidrains.

https://github.com/lucidrains/vector-quantize-pytorch/blob/master/vector_quantize_pytorch/residual_vq.py
"""
...
from torch import Tensor, arange, cat, nn, tensor
...
def frac_gradient(t, frac):
    ...
    return frac * t + (1.0 - frac) * detached
...
class ResidualVQ(Module):
    ...
    def __init__(
        ...
        **vq_kwargs,
        ...
        beam_score_quantizer_weights = default(beam_score_quantizer_weights, [1.0] * num_quantizers)
        ...
        if implicit_neural_codebook:
            self.mlps = ModuleList(
                [
                    MLP(dim=codebook_dim, l2norm_output=first(self.layers).use_cosine_sim, **mlp_kwargs)
    ...
    def forward(
        ...
        input_shape, num_quant, quant_dropout_multiple_of, return_loss, device = (
            x.shape,
            self.num_quantizers,
            self.quantize_dropout_multiple_of,
            exists(indices),
        ...
        for quantizer_index, (vq, maybe_mlp) in enumerate(zip(self.layers, maybe_code_transforms)):
            is_last_step = (
                (quantizer_index == (len(self.layers) - 1))
                if not should_quantize_dropout
                else quantizer_index == rand_quantize_dropout_index
            )
        ...
        if is_beam_search:
            quantized_out, all_indices, all_losses, all_residuals = [
                t[..., 0, :] for t in (quantized_out, all_indices, all_losses, all_residuals)
            ]

            # handle commit loss, which should be the average

            if exists(mask):
                all_losses = einx.where("..., ... l,", mask, all_losses, 0.0)
            ...
            if self.training:
                for vq, layer_input, indices in zip(
                    self.layers, all_residuals.unbind(dim=-2), all_indices.unbind(dim=-1)
                ):  # in the case of quantize dropout, zip will terminate with the shorter sequence, which should be all_residuals
